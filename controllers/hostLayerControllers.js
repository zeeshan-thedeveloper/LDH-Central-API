...
const addHostInRequestList = (req, res) => {
  ...
  host_users_schema.findOne({ hostId: hostId }, (err, data) => {
    console.log(data)
    if (data!=null ? data.connectedAdmin!=adminId : true) {
      const dataToInsert = {
      ...
      host_users_schema.create(dataToInsert, async (err, insertedData) => {
        ...
        if (!err) {
          ...
          const record = await admin_users_schema.findOneAndUpdate(
            ...
            {
              $push: {
                connectedHostList: { hostId: hostId,hostName:hostName }, //inserted data is the object to be inserted
              },
            },
            { new: true }
          );
          if (record) {
            ...
            res.status(200).send({
              responseMessage: "Successfully sent a connection request",
          ...
          res.status(501).send({
            responseMessage:
              "You have already requested this service manager",
        ...
        res.status(400).send({responseMessage : ' was not found',responseCode:COULD_NOT_DELETE});
         ...
         admin_users_schema.updateOne({ _id: adminId }, {
          ...
          if(data){
            ...
            res.status(200).send({responseMessage : ' Successfully deleted/disconnected',responseCode:DELETED});
          }else{
